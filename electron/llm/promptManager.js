...
class PromptManager {
  ...
  async getPrompt() {
    try {
      ...
      try {
        ...
        prompt = await fs.readFile(this.promptFilePath, 'utf-8');
        isCustom = true;
      } catch (error) {
        // File doesn't exist, use default prompt
        prompt = DEFAULT_MISTRAL_PROMPT;
        isCustom = false;
      }
      ...
      const availableTokens = contextSize - tokenCount;
      const usagePercent = Math.round((tokenCount / contextSize) * 100);
      ...
      return { 
        ...
        tokenInfo: {
          ...
          warning: usagePercent > 75 ? 'Prompt uses over 75% of context!' : null
        }
      };
    } catch (error) {
  ...
  // Get token info for any text (used by UI for real-time updates)
  async getTokenInfo(text) {
    ...
    return {
      ...
      availableTokens: availableTokens,
      usagePercent: usagePercent,
      warning: usagePercent > 75 ? 'Prompt uses over 75% of context!' : 
               usagePercent > 90 ? 'Critical: Prompt too long! May fail.' : null,
      status: usagePercent < 50 ? 'good' : 
              usagePercent < 75 ? 'warning' : 'danger'
    };
  }
}
